...
Original file is located at
    https://colab.research.google.com/drive/1wGWe-K5i-71YAbhVmxGIYmlcilRrspFv
"""
...
# get real images & create labels (file names are given)
def generate_real_samples(seed,n_samples):
	# retrieve selected images
	X = np.array(get_array_images(seed))
	X = X  / 255.0
...
def generate_seed(n_elements, n_samples):
  ...
  x_input = np.random.randint(0,200, size=(n_samples,n_elements))
  x_input -= 100
  return x_input
...
# get real images & create labels (file names must be genarated)
...
def summarize_performance(epoch, gan_model, g_model, d_model, dataset, n_elements, n_samples=100):
	...
	X_real, y_real = evaluate(n_samples)
...
def save_weights(g_model, d_model,epoch):
  filename = 'generator_model_' + str(epoch+1) + '.h5'
  g_model.save_weights(generate_path + 'models/' + filename)
  filename = 'discriminator_model_' + str(epoch+1) + '.h5'
  d_model.save_weights(generate_path + 'models/' + filename)

# train the generator and discriminator
def train(g_model, d_model, gan_model, dataset, n_elements, first, n_epochs=1000, n_batch=64):
	bat_per_epo = int(image_count / n_batch) * 2
	half_batch = int(n_batch / 2)
	# manually enumerate epochs
	for i in range(first,n_epochs):
		# enumerate batches over the training set
		real_seed = np.array(random.sample(range(image_count), half_batch * bat_per_epo))
		real_seed = np.reshape(real_seed,(bat_per_epo,half_batch))
		for j in range(bat_per_epo):
			# get randomly selected 'real' samples
			X_real, y_real = generate_real_samples(real_seed[j],half_batch)
...
#d_model.load_weights(generate_path + 'models/discriminator_model_100.h5')
#g_model.load_weights(generate_path + '111.h5')

d_model = compile_discriminator(d_model)
gan_model = define_gan(g_model, d_model)

# train model
train(g_model, d_model, gan_model, dataset, n_elements, 270)